...
/**
 * Helper function to navigate to inspection detail page and wait for load
 */
async function navigateToInspection(page: Page, loginPage: any) {
...
/**
 * Helper function to mock box update API endpoint
 * @param page - Playwright page object
 * @param behavior - 'always-fail' | 'fail-then-succeed' | 'delayed-fail'
 */
async function mockBoxUpdateApi(
  page: Page,
  behavior: 'always-fail' | 'fail-then-succeed' | 'delayed-fail'
) {
  let requestCount = 0;
...
/**
 * Helper function to trigger inspection save with box changes
 */
async function triggerInspectionSaveWithBoxChange(page: Page) {
...
/**
 * Helper function to get warning banner element
 */
function getWarningBanner(page: Page) {
  return page.getByRole('alert').filter({ 
    hasText: /(unable to update|box.{0,50}?configuration|failed)/i 
  });
}
...
test.describe('Inspection Box Update Retry Mechanism', () => {
  ...
  test('6.2: Box update fails during inspection save - warning banner appears', async ({ page, loginPage }) => {
    ...
    await navigateToInspection(page, loginPage);
    
    // Setup API mocking for box update to fail
    await mockBoxUpdateApi(page, 'always-fail');
    
    // Trigger inspection save with box changes
    await triggerInspectionSaveWithBoxChange(page);
    ...
    const warningBanner = getWarningBanner(page);
  ...
  test('6.3: Successful retry after initial failure - banner disappears', async ({ page, loginPage }) => {
    ...
    // Mock box update to fail first, then succeed
    await mockBoxUpdateApi(page, 'fail-then-succeed');
    ...
    // Verify button shows loading state (disabled during retry)
  ...
  test('6.6: In-progress state persists during navigation', async ({ page, loginPage }) => {
    ...
    await mockBoxUpdateApi(page, 'delayed-fail');
