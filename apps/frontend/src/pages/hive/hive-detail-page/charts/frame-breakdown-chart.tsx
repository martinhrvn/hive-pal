...
export const FrameBreakdownChart: React.FC<FrameBreakdownChartProps> = ({
  ...
  const [mode, setMode] = useState<ViewMode>('percent');
  ...
  const transform = useCallback(
    (inspection: InspectionResponse) => {
      ...
      for (const ft of FRAME_TYPES) {
        const count = obs?.[ft.obsKey] ?? null;
        if (mode === 'percent') {
          entry[ft.key] =
      ...
      <CardHeader>
        <div className="flex items-start justify-between gap-2">
          <div>
            ...
            <CardDescription>
              {mode === 'percent'
                ? 'Percentage of total frames by content over time'
                : 'Raw frame counts by content over time'}
            </CardDescription>
          </div>
