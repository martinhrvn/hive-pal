...
const TREND_FIELDS: {
  ...
  { key: 'honeyFrames',         shortLabel: 'Stores',   color: '#eab308', isFrameCount: true  },
...
export const HiveHeaderStats = ({ hiveId }: { hiveId: string }) => {
  ...
  const chips = useMemo<ChipData[]>(() => {
    ...
    if (withFrameData.length >= 2) {
      ...
      const previousCounts = frameFields.map(f => (previous.observations?.[f.key] as number | null | undefined) ?? 0);
      ...
      const previousPcts = largestRemainder(previousCounts, frameSum(previous.observations!));

      frameFields.forEach((field, i) => {
        // Skip fields with no data in either inspection
        if (currentCounts[i] === 0 && previousCounts[i] === 0) return;
        ...
        result.push({
          ...
          delta: currentPcts[i] - previousPcts[i],
          unit: '%',
        });
      });
    }
    ...
    for (const field of TREND_FIELDS.filter(f => !f.isFrameCount)) {
      ...
      const previous = withData[1].observations![field.key] as number;
