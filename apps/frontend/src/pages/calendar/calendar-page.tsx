...
import { format, startOfDay, addDays, isBefore, isSameDay, subDays, addWeeks, subWeeks } from 'date-fns';
import { Calendar as CalendarIcon, Clock, AlertTriangle, ChevronLeft, ChevronRight, ExternalLink } from 'lucide-react';
      ...
      <MainContent>
        <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            <div className="space-y-4">
              {days.map((day) => {
                const { inspections: dayInspections, standaloneActions } = getEventsForDay(day);
                const isToday = isSameDay(day, new Date());
                const totalEvents = dayInspections.length + standaloneActions.length;
                
                return (
                  <Card key={day.toISOString()} className={isToday ? 'ring-2 ring-blue-200' : ''}>
                        ...
                        {totalEvents > 0 && (
                          <div className="flex gap-2">
                            ...
                            {standaloneActions.length > 0 && (
                              <Badge variant="outline" className="text-xs">
                                {standaloneActions.length} action{standaloneActions.length > 1 ? 's' : ''}
                              </Badge>
                            )}
                          </div>
                        )}
                              ...
                              {inspection.actions && inspection.actions.length > 0 && (
                                <div className="mt-2 pt-2 border-t border-current/20">
                                  <p className="text-xs opacity-75">
                                    Actions: {inspection.actions.map(a => a.type).join(', ')}
                                  </p>
                                </div>
                              )}
                            </div>
