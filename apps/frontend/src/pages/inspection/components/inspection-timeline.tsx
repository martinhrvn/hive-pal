...
import { CalendarIcon, ChevronRightIcon, ActivityIcon, DropletsIcon, EggIcon, ChevronDownIcon } from 'lucide-react';
...
export const InspectionTimeline: React.FC<InspectionTimelineProps> = ({
  ...
  const formatDate = useCallback((date: Date | string): string => {
    ...
    if (isWithinInterval(parsedDate, { start: twoWeehsAgo, end: twoWeeksFromNow })) {
  ...
  return (
    <div className="space-y-4">
      {displayedInspections.length === 0 ? (
        <p className="text-muted-foreground text-center py-4">No inspections recorded yet.</p>
      ) : (
        <>
              ...
              <div className="p-4">
                ...
                <div className="grid grid-cols-3 gap-4 mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`font-semibold ${getStrengthColor(inspection.observations?.strength ?? null)}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`font-semibold ${getHoneyColor(inspection.observations?.honeyStores ?? null)}`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`font-semibold ${getBroodColor(calculateBroodScore(inspection) as number | null)}`}>
                ...
                {(inspection.temperature || inspection.weatherConditions) && (
                  <div className="text-sm text-muted-foreground mb-2">
                    Weather: {inspection.temperature ? `${inspection.temperature}°` : ''} 
                    {inspection.weatherConditions ? 
                      (inspection.temperature ? `, ${inspection.weatherConditions}` : inspection.weatherConditions) 
                      : ''}
                  </div>
                )}
                ...
                {inspection.observations?.queenSeen !== null && (
                  <div className="mt-2 text-sm">
                    ...
                    <span className={inspection.observations?.queenSeen ? "text-green-600" : "text-amber-500"}>
          ...
          {sortedInspections.length > MAX_DISPLAYED && (
            <div className="text-center mt-4">
              <Button 
                ...
                className="text-sm"
              >
                {showAll ? "Show fewer" : `Show all ${sortedInspections.length} inspections`}
                <ChevronDownIcon className={`ml-2 h-4 w-4 transition-transform ${showAll ? 'rotate-180' : ''}`} />
              </Button>
            </div>
          )}
        </>
